...
def convert(audio_picker,model_picker,index_picker,index_rate,pitch,method):
    ...
    command = [
        ...
        "--f0method", method,
        "--opt_path", f"audios/{audioss_name}_{names_model}_ver.wav",
        ...
        with gr.Column():
            with gr.Tabs():
                with gr.TabItem("1.Choose a voice model:"):
                    ...
                    names_model = gr.Textbox(label="model name must be the same as the pick Model")
                    audioss_model = gr.Textbox(label="audio name")
